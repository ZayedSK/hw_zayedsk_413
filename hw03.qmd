...
# Step 1: Read the Wikipedia page and extract the table
...
# Step 2: Remove the first (repeated header) row and assign proper column names
pop_clean <- pop_raw[-1, ]

colnames(pop_clean) <- c(
  "state_or_territory",
  "census_population_2024",
...
# Step 3: Replace "__" with NA
pop_clean <- pop_clean %>%
  mutate(across(everything(), ~na_if(.x, "__")))

# Step 4: Check structure and preview first few rows
str(pop_clean)
head(pop_clean)
...
# Connect to the database again (ensure correct path)
practcon <- dbConnect(duckdb(), dbdir = "flights.duckdb", read_only = FALSE)

# List all tables using R, not SQL

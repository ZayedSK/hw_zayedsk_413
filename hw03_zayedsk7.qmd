...
#The Data would not upload properly and everytime i tried to show the table it would say 0 rows so i went on [https://data-science-master.github.io/lectures/data](https://data-science-master.github.io/lectures/data/flights.duckdb){.uri} and took the information and asked ChatGPT to help me to come up with this solution
...
3.  Replace `__` with `NA`\
    ...
    # Step 3: Clean column names and fix header row
    pop_df <- pop_df[-1, ] %>%    # Remove the duplicate header row
      clean_names() %>%
